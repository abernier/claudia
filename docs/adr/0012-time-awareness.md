...
injected _date_ (no time), and session artifacts were stamped to the day. So when
...
derived from the _local_ hour for the same reason.
...
morning), _combined with_ a crossed calendar day — so it catches both the ordinary
...
Claudia is _exact_ about time (she will not say "good evening" in the morning) but
_sober_ about the gap. On `overnight` / `multi_day` she may acknowledge the pause
**once**, warmly, in the opening — naming the _gap_, never _how_ it happened (no
...
presence, not surveillance — so this is deliberately _not_ to be "improved" into
...
- Known limitation: in a _fresh_ (non-continued) session the persona signature is
  ...
  first message back.
- Persona guidance lives in `skills/claudia/SKILL.md` (_sense of time_) and
