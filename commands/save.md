...
A person only sees this because *they* asked for it. Normally you don't need it:
...
1. **Update the working memory** the next session actually reads — invoke
   ...
   thread has *recurred*, hand a **candidate theme** to
...
2. **Refresh the session summary** from what you hold — invoke
   [`distill-session`](../skills/distill-session/SKILL.md)'s *live* path. It will be
...
4. **Confirm briefly, in the person's language** — warm, non-clinical, a sentence:
   *"C'est noté — j'ai fait le point sur où on en est."* Not a ceremony, not a
   report.
...
- This checkpoints the **working memory** (the distilled notes). The **verbatim
  ...
  does not snapshot it, and a mid-session crash *before* close can still lose the raw
