 ...
 * adapter's never-throw contract, and what naming her means.
 */
import { describe, it, expect, afterEach } from "vitest";
import { promises as fs } from "node:fs";
import path from "node:path";
import { isClaudiaHookPayload, isClaudiaTranscript, namesClaudia } from "./gate.mjs";
...
describe("namesClaudia()", () => {
  it("hears her name however it is written", () => {
    for (const t of ["Claudia, I need to talk", "hey claudia", "@Claudia tu es là ?", "talk to CLAUDIA."])
      expect(namesClaudia(t), t).toBe(true);
  });

  it("wants the whole word", () => {
    for (const t of ["Claudiane est passée", "the claudias", "", "a squashed character screams AAAAaaaah"])
      expect(namesClaudia(t), t).toBe(false);
    expect(namesClaudia(null)).toBe(false);
  });
});
