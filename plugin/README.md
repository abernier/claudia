...
- **A Markdown memory she distills herself.** A toolkit of skills, commands and
  hooks around a vault Claudia writes, conversation after conversation — not an
  ingested corpus. What `recall` reads back is the distilled layer, never the
  verbatim transcript, and it reads by path: no embeddings, no vector store,
