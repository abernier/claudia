...
- **The safety hook also hears her name.** The `claudia` skill triggers when the
  person names her, so on turn one the activation is not in the transcript yet.
  A prompt that names her — "Claudia", "@Claudia", any case, whole word — opens
  the gate for that turn, so a crisis in the very first "Claudia, …" message is
  screened.
- **The safety hook screens the person's words only.** Harness blocks —
  ...
  is not screened and does not count as naming her. Slash-command arguments stay
...
- **The accepted gap.** A first message carrying crisis content that neither names
  Claudia nor follows her activation is not screened by the hook — there is no way
...
- **The name is broad on purpose.** A path such as `~/code/claudia/` names her, so
  a coding session in this very repository is screened on those turns. That
  over-reach is the right direction for a safety check: a note too many costs a
  note, a missed crisis costs far more.
